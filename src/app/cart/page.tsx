...
function Cart() {
  const { cart, totalAmount, removeFromCart, updateQuantity, updateDeposit, updateDuration } = useCart();

  const fixedDeposit = 5000; // Fixed deposit amount

  useEffect(() => {
    // Optionally, you can recalculate the total amount if needed.
    // The totalAmount should be managed in the CartProvider now.
  }, [cart]);
                ...
                <div>
                  ...
                  <div className="mb-2">
                    ...
                    <input
                      type="number"
                      value={fixedDeposit}
                      onChange={(e) => updateDeposit(item.id, parseFloat(e.target.value))}
